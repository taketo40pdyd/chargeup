...
function buildNotification(ctx, payload) {
  ...
  const lines = [];
  if (ctx && ctx.cyc && ctx.cyc.name) {
    const c = ctx.cyc;
    lines.push(c.kind === 'due'
      ? `「${c.name}」の買い替え目安（${c.life}年）を過ぎました。`
      : `「${c.name}」の買い替え目安（${c.life}年）が近づいています。`);
  }
  if (ctx && ctx.name) {
  ...
  if (lines.length) return { title: 'ChargeUp', body: lines.join('\n'), tag: 'chargeup-reminder' };
